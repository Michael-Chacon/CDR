			...
			<?php if ($actividades_estudiantes->rowCount() != 0): ?>
					<?php while ($actividad = $actividades_estudiantes->fetchObject()): ?>
							<div class="card shadow mt-3 mb-3" style="border-left: 6px solid <?=$actividad->color?> !important;">
								...
								<div class="collapse multi-collapse" id="info_estudiante<?=$actividad->id?>">
					...
					<span class="d-grid gap-2 p-3">
						<a  href="<?=base_url?>Tablero/verActividades&user=<?=Utils::encryption("estudiante")?>" type="button" class="btn btn-outline-primary position-relative btn-sm">
